...
/**
 * @title SpokePoolTron
 * @notice Tron-compatible fork of SpokePool. Resolves `isContract` ambiguity between AddressLibUpgradeable
 * and OZ's AddressUpgradeable (from SafeERC20Upgradeable) for solc 0.8.25 compatibility.
 * @dev Only difference from SpokePool.sol: two `.isContract()` calls replaced with explicit
 * `AddressLibUpgradeable.isContract()` to resolve the ambiguity that causes compilation failure on solc <0.8.26.
 * @custom:security-contact bugs@across.to
 */
abstract contract SpokePoolTron is
