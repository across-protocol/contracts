...
6. **Bounded trust.** The policy owner is a meaningful authority but cannot redirect destination, output token, or recipient (clone immutables guard those). Fee bounds are committed in the merkle tree. The clone's `userAddress` retains a structurally-guaranteed escape — full execution authority over the clone, bypassing the policy entirely. `userAddress` is also the forced destination for `WithdrawImplementation`, so manager-driven withdrawals can be authorized by lower-trust roles (`directWithdrawer`, `signer`) without giving those roles the power to redirect funds.
...
Recipients are clone-immutable in both cases. A compromised `directWithdrawer` or `signer` on the manager can force a withdrawal but cannot redirect it.
...
- [x] **`AdminWithdrawManager.sol`** — `SIGNED_WITHDRAW_TYPEHASH` is `SignedWithdraw(address depositAddress, address withdrawImpl, address token, uint256 amount, uint256 deadline)`; recipient is not signed because it's clone-immutable, but `withdrawImpl` is signed so a submitter cannot redirect to a different impl. Both withdraw paths take `withdrawImpl` and `bytes32[] calldata proof` as arguments (no immutable on the manager — avoids a circular construction dep with the impl). Each path calls `clone.execute(cloneArgs, withdrawImpl, "", abi.encode(token, amount), proof)`. The dispatcher verifies the proof and the impl's `admin == manager` arm authorizes the call. Caller-supplied `cloneArgs` is required since the manager doesn't hold per-clone state.
