...
- [x] **`CounterfactualDepositCCTP.sol`** — adopt the new interface signature; add `signer` constructor immutable, `ExecuteCCTP` EIP-712 typehash, signature verification over `(clone, paramsHash, amount, executionFee, executionFeeDeadline)`, new submitter-data fields, `maxExecutionFee` leaf field (fixed-amount cap on `executionFee`), and the standardized leaf-params prefix.
