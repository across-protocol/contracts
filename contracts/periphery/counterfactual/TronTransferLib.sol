 ...
 * @dev Reverts with `TronTransferCallReverted` if the underlying call reverts, or
 *      `TronTransferBalanceMismatch` if the recipient's balance does not increase by
 *      exactly `amount`. Assumes no fee-on-transfer.
 */
library TronTransferLib {
    ...
    function _safeTransferBalanceCheck(address token, address to, uint256 amount) internal {
        ...
        if (IERC20(token).balanceOf(to) != pre + amount) revert TronTransferBalanceMismatch();
    }
}
