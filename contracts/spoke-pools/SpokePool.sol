    ...
    IAcrossV5Executor
{
    ...
    // V5 Gateway permitted to drive `executeAcrossV5Msg`. address(0) disables the V5 intent path on this deployment.
    /// @custom:oz-upgrades-unsafe-allow state-variable-immutable
    ...
    // first 32 bytes equal this prefix may only originate from the V5 execute path (`executeAcrossV5Msg`), which is
     ...
     * @param _gateway V5 Gateway permitted to drive `executeAcrossV5Msg`. Pass address(0) to disable the V5 path.
     */
    ...
    function _fillV5(
        ...
        AcrossV5FillJit memory jit = abi.decode(submitterMsg, (AcrossV5FillJit));

        if (jit.witness == bytes32(0) || jit.witness != IAcrossV5Gateway(gateway).currentStepId())
            revert WitnessMismatch();
        // Over-delivery (>= floor) lets the submitter sponsor the recipient's tape.
        if (jit.resolvedOutputAmount < f.outputAmount)
            revert OutputBelowMinimum(f.outputAmount, jit.resolvedOutputAmount);

        // Build relay data + run the shared commit pipeline (deadline + exclusivity + status dedup + FilledRelay event).
        V3RelayData memory relayData = _commitV5Fill(f, jit, submitter);

        _transferTokensToRecipientV5(
            relayData,
            submitter,
            jit.resolvedOutputAmount,
            recipientMsg,
            jit.recipientJit,
            nativeValue
        );
    ...
    // Construct the V5 relay data, hash it, and run the shared `_commitFill` pipeline. Separate frame to stay under the
    // stack limit; returns the relay data so the caller can deliver funds.
    function _commitV5Fill(
        AcrossV5FillData memory f,
        AcrossV5FillJit memory jit,
        address submitter
    ) internal returns (V3RelayData memory relayData) {
        relayData = V3RelayData({
            ...
            message: abi.encodePacked(MAGIC_ACXV5_MESSAGE_PREFIX, jit.witness)
        });
        ...
        _commitFill(
            V3RelayExecutionParams({
                relay: relayData,
                relayHash: getV3RelayHash(relayData),
                updatedOutputAmount: jit.resolvedOutputAmount,
                updatedRecipient: relayData.recipient,
                updatedMessage: relayData.message,
                repaymentChainId: jit.repaymentChainId
            }),
