    ...
    IDestinationSettler
{
    ...
    bytes32 public constant UPDATE_BYTES32_DEPOSIT_DETAILS_HASH =
        keccak256(
            "UpdateDepositDetails(bytes32 relayHash,uint256 updatedOutputAmount,bytes32 updatedRecipient,bytes updatedMessage)"
        );
     ...
     * and/or message.
     * @dev The relay data hash must match the hash of the params in a FundsDeposited event that the depositor
     * wants to speed up and the potential FilledRelay event that will be generated by a relayer.
     * The relayer has the option but not the obligation to use this updated information
     ...
     * @param relayHash Unique relay data hash to speed up
     ...
     * been mined.
     *
     * @dev The relayHash must match the hash of the parameters in a `FundsDeposited` event that the depositor wants
     * to speed up as well as the hash of the parameters in a prospective `FilledRelay` event generated by a relayer.
    ...
    function _verifyUpdateV3DepositMessage(
        ...
        bytes32 expectedTypedDataV4Hash = _hashTypedDataV4(
            keccak256(
                abi.encode(hashType, relayHash, updatedOutputAmount, updatedRecipient, keccak256(updatedMessage))
            ),
            originChainId
        );
