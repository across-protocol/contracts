...
contract LspUniswapV2Broker {
    ...
    function atomicMintAddLiquidity(
        ...
        uint256 tokensToCreate = FixedPoint
            .fromUnscaledUint(amountCollateral)
            .div(longShortPair.collateralPerPair())
            .rawValue;
    ...
    function computeSwapToMintAtPoolRatio(
        ...
        FixedPoint.Signed memory numerator1 = sqrt(
            rb.mul(ra.add(m)).mul(
                ra.mul(rb.mul(lambda2().add(num(2).mul(lambda())).add(num(1))).add(num(4).mul(lambda().mul(m)))).add(
                    rb.mul(lambda2().sub(num(2).mul(lambda())).add(num(1))).mul(m)
                )
            )
        );
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (IUniswapV2Pair pair) {
