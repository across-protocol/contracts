    ...
    Lockable
{
    ...
    function _requestPriceSpoke(bytes32 identifier, uint256 time, bytes memory ancillaryData) internal {
    ...
    function processMessageFromParent(bytes memory data) public override nonReentrant onlyMessenger {
        (bytes32 identifier, uint256 time, bytes memory ancillaryData, int256 price) = abi.decode(
            data,
     ...
     */
    function resolveLegacyRequest(bytes32 identifier, uint256 time, bytes memory ancillaryData) external {
