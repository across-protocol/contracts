...
contract OracleHub is OracleBase, ParentMessengerConsumerInterface, Ownable, Lockable, MultiCaller {
    ...
    function processMessageFromChild(
        uint256 chainId,
        bytes memory data
    ) public override nonReentrant onlyMessenger(chainId) {
     ...
     */
    function requestPrice(bytes32 identifier, uint256 time, bytes memory ancillaryData) public nonReentrant {
        bool newPriceRequested = _requestPrice(identifier, time, ancillaryData);
        if (newPriceRequested) {
