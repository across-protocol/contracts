...
contract PricelessPositionManager is FeePayer {
    ...
    ) public onlyPreExpiration noPendingWithdrawal(msg.sender) nonReentrant {
        require(
    ...
    function repay(
        FixedPoint.Unsigned memory numTokens
    ) public onlyPreExpiration noPendingWithdrawal(msg.sender) fees nonReentrant {
        PositionData storage positionData = _getPositionData(msg.sender);
    ...
    function redeem(
        FixedPoint.Unsigned memory numTokens
    ) public noPendingWithdrawal(msg.sender) fees nonReentrant returns (FixedPoint.Unsigned memory amountWithdrawn) {
        PositionData storage positionData = _getPositionData(msg.sender);
