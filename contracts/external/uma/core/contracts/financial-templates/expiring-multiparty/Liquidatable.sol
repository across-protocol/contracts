...
contract Liquidatable is PricelessPositionManager {
    ...
    function dispute(
        uint256 liquidationId,
        address sponsor
    ) external disputable(liquidationId, sponsor) fees nonReentrant returns (FixedPoint.Unsigned memory totalPaid) {
        ...
        FixedPoint.Unsigned memory disputeBondAmount = disputedLiquidation
            ...
            .mul(_getFeeAdjustedCollateral(disputedLiquidation.rawUnitCollateral));
    ...
    function withdrawLiquidation(
        uint256 liquidationId,
        address sponsor
    ) public withdrawable(liquidationId, sponsor) fees nonReentrant returns (RewardsData memory) {
    ...
    function getLiquidations(
        address sponsor
    ) external view nonReentrantView returns (LiquidationData[] memory liquidationData) {
    ...
    function _settle(uint256 liquidationId, address sponsor) internal {
        ...
        FixedPoint.Unsigned memory requiredCollateral = tokenRedemptionValue.mul(
            _transformCollateralRequirement(liquidation.settlementPrice)
        );
    ...
    function _getLiquidationData(
        address sponsor,
        uint256 liquidationId
    ) internal view returns (LiquidationData storage liquidation) {
    ...
    function _transformCollateralRequirement(
        FixedPoint.Unsigned memory price
