...
abstract contract FeePayer is AdministrateeInterface, Testable, Lockable {
     ...
     */
    constructor(address _collateralAddress, address _finderAddress, address _timerAddress) Testable(_timerAddress) {
    ...
    ) internal view returns (FixedPoint.Unsigned memory collateral) {
    ...
    ) internal view returns (FixedPoint.Unsigned memory rawCollateral) {
