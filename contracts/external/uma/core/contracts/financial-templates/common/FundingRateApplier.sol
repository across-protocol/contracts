...
abstract contract FundingRateApplier is EmergencyShutdownable, FeePayer {
     ...
     */
    function proposeFundingRate(
        FixedPoint.Signed memory rate,
        uint256 timestamp
    ) external fees nonReentrant returns (FixedPoint.Unsigned memory totalBond) {
    ...
    function _getFundingRateAppliedTokenDebt(
        FixedPoint.Unsigned memory rawTokenDebt
    ) internal view returns (FixedPoint.Unsigned memory tokenDebt) {
        ...
        if (proposalTime != 0) {
            ...
            try optimisticOracle.settleAndGetPrice(identifier, proposalTime, ancillaryData) returns (int256 price) {
                ...
                if (proposalTime >= lastUpdateTime) {
                    ...
                    OptimisticOracleInterface.Request memory request = optimisticOracle.getRequest(
                        address(this),
                        identifier,
                        proposalTime,
                        ancillaryData
