...
contract PerpetualPositionManager is FundingRateApplier {
    ...
    ) public notEmergencyShutdown noPendingWithdrawal(sponsor) fees nonReentrant {
    ...
    function requestWithdrawal(
        FixedPoint.Unsigned memory collateralAmount
    ) public notEmergencyShutdown noPendingWithdrawal(msg.sender) nonReentrant {
        PositionData storage positionData = _getPositionData(msg.sender);
        require(
    ...
    function repay(
        FixedPoint.Unsigned memory numTokens
    ) public notEmergencyShutdown noPendingWithdrawal(msg.sender) fees nonReentrant {
        PositionData storage positionData = _getPositionData(msg.sender);
        require(numTokens.isLessThanOrEqual(positionData.tokensOutstanding));
    ...
    function getCollateral(
        address sponsor
    ) external view nonReentrantView returns (FixedPoint.Unsigned memory collateralAmount) {
    ...
    ) external view nonReentrantView returns (FixedPoint.Unsigned memory totalCollateral) {
    ...
    function _getPositionData(
        address sponsor
    ) internal view onlyCollateralizedPosition(sponsor) returns (PositionData storage) {
