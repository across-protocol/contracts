...
contract LongShortPairCreator is Testable, Lockable {
     ...
     */
    constructor(FinderInterface _finder, TokenFactory _tokenFactory, address _timer) Testable(_timer) nonReentrant() {
    ...
    function createLongShortPair(CreatorParams memory params) public nonReentrant returns (address) {
        ...
        ExpandedIERC20 longToken = tokenFactory.createToken(
            params.longSynthName,
            params.longSynthSymbol,
            collateralDecimals
        );
        ExpandedIERC20 shortToken = tokenFactory.createToken(
            params.shortSynthName,
            params.shortSynthSymbol,
            collateralDecimals
        );
