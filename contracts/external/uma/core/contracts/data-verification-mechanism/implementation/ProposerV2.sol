...
contract ProposerV2 is Ownable, Lockable, MultiCaller {
     ...
     */
    constructor(IERC20 _token, uint256 _bond, GovernorV2 _governor, Finder _finder) {
        token = _token;
        governor = _governor;
        finder = _finder;
        setBond(_bond);
     ...
     */
    function propose(
        GovernorV2.Transaction[] memory transactions,
    ...
    function resolveProposal(uint256 id) external nonReentrant {
        ...
        OracleAncillaryInterface voting = OracleAncillaryInterface(
