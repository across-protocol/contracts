...
contract VotingV2 is Staker, OracleInterface, OracleAncillaryInterface, OracleGovernanceInterface, VotingV2Interface {
    ...
    ****************************************/
    ...
    function _requestPrice(bytes32 identifier, uint256 time, bytes memory ancillaryData, bool isGovernance) internal {
    ...
    function revealVote(
        ...
        VoteInstance storage voteInstance = _getPriceRequest(identifier, time, ancillaryData).voteInstances[
            currentRoundId
        ];
        address voter = getVoterFromDelegate(msg.sender);
    ...
    function getPendingRequests() public view override returns (PendingRequestAncillaryAugmented[] memory) {
        ...
        PendingRequestAncillaryAugmented[] memory unresolved = new PendingRequestAncillaryAugmented[](
        ...
        PendingRequestAncillaryAugmented[] memory pendingRequests = new PendingRequestAncillaryAugmented[](
            numUnresolved
        );
    ...
    function requestSlashingTrackers(uint256 requestIndex) public view returns (SlashingTracker memory) {
        ...
        (uint256 wrongVoteSlash, uint256 noVoteSlash) = rounds[lastVotingRound].slashingLibrary.calcSlashing(
            totalStaked,
            totalVotes,
            totalCorrectVotes,
            requestIndex,
    ...
    function getVoterParticipation(
        ...
        VoteInstance storage voteInstance = priceRequests[resolvedPriceRequestIds[requestIndex]].voteInstances[
            lastVotingRound
        ];
    ...
    function retrieveRewardsOnMigratedVotingContract(
        ...
        uint256 rewards = MinimumVotingAncillaryInterface(address(previousVotingContract))
    ...
    function _getPriceOrError(
        ...
        (bool previouslyResolved, int256 previousPrice) = _getPriceFromPreviousVotingContract(
            identifier,
            time,
            ancillaryData
        );
