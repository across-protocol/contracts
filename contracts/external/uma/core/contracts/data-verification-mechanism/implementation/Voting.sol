    ...
    VotingAncillaryInterface // Interface to support ancillary data with voting rounds.
{
    ...
    ****************************************/
    ...
    ) external override(VotingInterface, VotingAncillaryInterface) onlyIfNotMigrated {
        uint256 blockTime = getCurrentTime();
    ...
    // Overloaded method to enable short term backwards compatibility. Will be deprecated in the next DVM version.
    function revealVote(bytes32 identifier, uint256 time, int256 price, int256 salt) public override {
    ...
    function setMigrated(
        address newVotingAddress
    ) external override(VotingInterface, VotingAncillaryInterface) onlyOwner {
    ...
    ) public override(VotingInterface, VotingAncillaryInterface) onlyOwner {
    ...
    function _getPriceOrError(
        ...
        uint256 time,
        bytes memory ancillaryData
    ) private view returns (bool, int256, string memory) {
        PriceRequest storage priceRequest = _getPriceRequest(identifier, time, ancillaryData);
        uint256 currentRoundId = voteTiming.computeCurrentRoundId(getCurrentTime());
