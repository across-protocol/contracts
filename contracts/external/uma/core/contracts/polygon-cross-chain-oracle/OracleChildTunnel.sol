...
contract OracleChildTunnel is OracleBaseTunnel, OracleAncillaryInterface, FxBaseChildTunnel, Lockable {
    ...
    ) OracleBaseTunnel(_finderAddress) FxBaseChildTunnel(_fxChild) {}
    ...
    function resolveLegacyRequest(
        ...
        bytes32 legacyRequestId = _encodePriceRequest(
            identifier,
            time,
            _legacyStampAncillaryData(ancillaryData, childRequester)
        );
     ...
     */
    function _legacyStampAncillaryData(
        bytes memory ancillaryData,
        address requester
