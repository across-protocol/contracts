...
Gateway does not currently enforce this net-zero settlement invariant. A residual delegate allowance is not
privileged—the next permissionless `APPROVE` may replace it—but the SpokePool never delegates its own vault.
