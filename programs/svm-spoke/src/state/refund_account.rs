...
impl<'c, 'info> RefundAccount<'info> where 'c: 'info {
  ...
  pub fn try_from_remaining_account(
    ...
    Self::try_token_account_from_account_info(refund_account_info, expected_token_account, expected_mint, token_program)
      .map(Self::TokenAccount)
      .or_else(|| {
        Self::try_claim_account_from_account_info(refund_account_info, expected_mint, expected_token_account).map(
          Self::ClaimAccount
        )
      })
      .ok_or_else(|| {
        error::Error::from(CustomError::InvalidRefund).with_account_name(&format!("remaining_accounts[{}]", index))
      })
  }
