...
async function main() {
  const spokePoolArtifact = findArtifactFromPath("Arbitrum_SpokePool", ARTIFACTS_PATH);
  ...
  for (const token of tokenSymbols) {
    ...
    const _callData = spokePoolInterface.encodeFunctionData("whitelistToken", [arbitrumToken, l1Token]);
