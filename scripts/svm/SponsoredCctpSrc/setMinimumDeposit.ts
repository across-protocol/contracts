// This script sets new minimum deposit amount for a supported burnToken on a SVM Sponsored CCTP bridge.
...
import { getSponsoredCctpSrcPeripheryProgram } from "@across-protocol/sdk/svm";
...
const argv = yargs(hideBin(process.argv))
  .option("burnToken", {
    type: "string",
    demandOption: true,
    describe: "Burn token mint address to set minimum deposit for",
  })
  .option("amount", {
    type: "number",
    demandOption: false,
    default: 0,
    describe: "Minimum deposit amount (raw value)",
  }).argv;

async function setMinimumDeposit(): Promise<void> {
  const resolvedArgv = await argv;
  const burnToken = new PublicKey(resolvedArgv.burnToken);
  const amount = new anchor.BN(resolvedArgv.amount);
  ...
  console.log("Setting new minimum deposit amount...");
  console.table([
  ...
  const ix = await program.methods
    .setMinimumDepositAmount({ amount })
    .accounts({
  ...
  if (txSigner.equals(upgradeAuthority)) {
    console.log("Signer is the upgrade authority, setting new quote signer...");
