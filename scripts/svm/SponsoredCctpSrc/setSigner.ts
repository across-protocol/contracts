...
import { evmAddressToPublicKey, getSponsoredCctpSrcPeripheryProgram } from "@across-protocol/sdk/dist/cjs/src/svm";
...
// Parse arguments
const argv = yargs(hideBin(process.argv)).option("quoteSigner", {
  type: "string",
  demandOption: true,
  describe: "New sponsored deposit quote signer, EVM address",
}).argv;
...
async function setSigner(): Promise<void> {
  ...
  console.log("Setting new quote signer...");
  console.table([
  ...
  const ix = await program.methods
    .setSigner({ newSigner: quoteSigner })
    .accounts({
  ...
  if (txSigner.equals(upgradeAuthority)) {
    ...
    console.log("Quote signer set successfully, transaction signature:", txSignature);
  } else {
    console.log(
