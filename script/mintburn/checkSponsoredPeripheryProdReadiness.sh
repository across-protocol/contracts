...
source "$ROOT_DIR/scripts/setupFoundryEnv.sh"
...
check_donation_box_role_based() {
	...
	if ! try_cast_view parent_has_withdrawer "$rpc_url" "$donation_box" "hasRole(bytes32,address)(bool)" "$withdrawer_role" "$parent_address"; then
	...
	elif [[ "$parent_has_withdrawer" != "true" ]]; then
		add_finding \
			"WARN" \
			"$chain_id $chain_name" "DonationBox" "$donation_box" "WITHDRAWER_ROLE(parent)" "missing $parent_address"
		propose_grant_role \
			50 \
			"$chain_id" \
			"$chain_name" \
		...
		add_detail "$chain_id $chain_name" "DonationBox" "$donation_box" "WITHDRAWER_ROLE(parent)" "$parent_address"
	fi

	# Dev wallet must not hold WITHDRAWER_ROLE.
	local dev_has_withdrawer
	if ! try_cast_view dev_has_withdrawer "$rpc_url" "$donation_box" "hasRole(bytes32,address)(bool)" "$withdrawer_role" "$DEV_WALLET"; then
		add_finding "ERROR" "$chain_id $chain_name" "DonationBox" "$donation_box" "hasRole(WITHDRAWER_ROLE,dev)" "query failed"
	elif [[ "$dev_has_withdrawer" == "true" ]]; then
		add_finding \
			"FAIL" \
			"$chain_id $chain_name" "DonationBox" "$donation_box" "WITHDRAWER_ROLE" "dev wallet present"
		propose_revoke_role \
			40 \
			"$chain_id" \
			"$chain_name" \
		...
		add_detail "$chain_id $chain_name" "DonationBox" "$donation_box" "WITHDRAWER_ROLE(dev)" "absent"
	fi

	# Dev wallet must not hold DEFAULT_ADMIN_ROLE; it should be the chain multisig.
	local dev_is_admin
	if ! try_cast_view dev_is_admin "$rpc_url" "$donation_box" "hasRole(bytes32,address)(bool)" "$default_admin_role" "$DEV_WALLET"; then
		add_finding "ERROR" "$chain_id $chain_name" "DonationBox" "$donation_box" "hasRole(DEFAULT_ADMIN_ROLE,dev)" "query failed"
		return
	fi
	...
	if [[ "$dev_is_admin" == "true" ]]; then
		add_finding \
			"FAIL" \
			"$chain_id $chain_name" "DonationBox" "$donation_box" "DEFAULT_ADMIN_ROLE" "dev wallet present"
		propose_default_admin_handoff "$chain_id" "$chain_name" "DonationBox" "$donation_box" "$default_admin_role"
	elif [[ "$FULL_REPORT" -eq 1 ]]; then
