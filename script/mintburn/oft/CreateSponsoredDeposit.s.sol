...
forge script script/mintburn/oft/CreateSponsoredDeposit.s.sol:CreateSponsoredDeposit \
  --sig "run(string,uint256)" usdt0 1000000 --rpc-url arbitrum -vvvv

forge script script/mintburn/oft/CreateSponsoredDeposit.s.sol:CreateSponsoredDeposit \
  --sig "run(string,uint256,address)" usdt0 1000000 0xRecipient --rpc-url arbitrum -vvvv
*/
contract CreateSponsoredDeposit is Script, Config {
    ...
    function run() external pure {
        revert("use run(str,uint256[,addr])");
    }
    ...
    function run(string memory tokenName, uint256 amountLD) external {
        ...
        address finalRecipient = deployer; // default to signer address
        _execute(env, pk, deployer, amountLD, finalRecipient);
    }

    function run(string memory tokenName, uint256 amountLD, address finalRecipient) external {
        require(bytes(tokenName).length != 0, "token key required");
        string memory configPath = string(abi.encodePacked("./script/mintburn/oft/", tokenName, ".toml"));
        _loadConfigAndForks(configPath, false);
        DepositEnv memory env = _resolveEnv();
        ...
        _execute(env, pk, deployer, amountLD, recipient);
    }
    ...
    function _execute(
        ...
        SignedQuoteParams memory signedParams = SignedQuoteParams({
            ...
            finalToken: env.finalToken.toBytes32(),
    ...
    function _resolveEnv() internal returns (DepositEnv memory env) {
        ...
        // Pick the first configured chain that's not the source as destination
        ...
        require(dstChainId != 0, "dst chain not found in config");
        ...
        env.finalToken = config.get("token").toAddress();
        address dstMessenger = config.get("oft_messenger").toAddress();
        require(
            env.destinationHandler != address(0) && env.finalToken != address(0) && dstMessenger != address(0),
            "missing dst config"
        );
