...
if command -v ts-node &> /dev/null; then
    ...
    ts-node "$PROJECT_ROOT/script/ExtractDeployedFoundryAddresses.ts"
elif command -v npx &> /dev/null; then
    echo "Using npx ts-node to run TypeScript script..."
    npx ts-node "$PROJECT_ROOT/script/ExtractDeployedFoundryAddresses.ts"
else
...
echo "  - $PROJECT_ROOT/broadcast/DeployedAddresses.sol (Foundry smart contract with all addresses)"
echo ""
