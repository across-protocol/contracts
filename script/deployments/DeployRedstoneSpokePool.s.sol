...
/**
 * @title DeployRedstoneSpokePool
 * @notice Template for deploying any L2 SpokePool.
 * @dev Replace Redstone with the specific L2 name (e.g., Arbitrum, Optimism).
 */
contract DeployRedstoneSpokePool is Script, ChainUtils {
    ...
    function run() external {
        ...
        address wrappedNativeToken = getRedstoneWrappedNative(chainId);
        ...
        console.log("Deploying Redstone_SpokePool on chain %s", chainId);
    ...
    function getRedstoneWrappedNative(uint256 chainId) internal pure returns (address) {
