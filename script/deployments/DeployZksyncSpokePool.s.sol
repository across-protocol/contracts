...
/**
 * @title DeployZksyncSpokePool
 * @notice Template for deploying any L2 SpokePool.
 * @dev Replace Zksync with the specific L2 name (e.g., Arbitrum, Optimism).
 */
contract DeployZksyncSpokePool is Script, ChainUtils {
    ...
    function run() external {
        ...
        address wrappedNativeToken = getZksyncWrappedNative(chainId);
        ...
        console.log("Deploying Zksync_SpokePool on chain %s", chainId);
        ...
        // );
        // console.log("Zksync_SpokePool implementation deployed at: %s", address(spokePoolImplementation));

        // Deploy ProxyAdmin contract to manage the proxyn        ProxyAdmin proxyAdmin = new ProxyAdmin();n        console.log("ProxyAdmin deployed at: %s", address(proxyAdmin));n        n        // Create initialization data for the proxyn        bytes memory initData = abi.encodeWithSelector(n            Zksync_SpokePool.initialize.selector,n            INITIAL_DEPOSIT_ID,  // Initial deposit IDn            hubPoolAddress,      // Set hub pool as cross domain adminn            hubPoolAddress       // Set hub pool as withdrawal recipientn        );n        n        // Deploy the proxy pointing to the implementation with initialization datan        TransparentUpgradeableProxy proxy = new TransparentUpgradeableProxy(n            address(spokePoolImplementation),n            address(proxyAdmin),     // Admin of the proxyn            initDatan        );n        console.log("Zksync_SpokePool proxy deployed at: %s", address(proxy));n        n        // Transfer ProxyAdmin ownership to the deployern        proxyAdmin.transferOwnership(deployer);n        console.log("ProxyAdmin ownership transferred to: %s", deployer);        // Note: In a real deployment, you would:
    ...
    function getZksyncWrappedNative(uint256 chainId) internal pure returns (address) {
