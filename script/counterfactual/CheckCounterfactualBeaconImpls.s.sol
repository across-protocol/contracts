...
contract CheckCounterfactualBeaconImpls is CounterfactualConfig, CheckUtils {
    ...
    function _checkTokens(uint256 chainId, CounterfactualChainConfig memory c) internal {
        // A chain that pins its USDC slot via `[N.address] usdcOverride` (e.g. pathUSD on Tempo) is checked by
        // ADDRESS, not symbol: the override names the exact token, which is a stronger assertion than a symbol
        // allowlist and — unlike `symbol()` — stays conclusive for the TIP-20 system tokens these overrides
        // typically name. Chains without an override keep the symbol check.
        address usdcOverrideAddr = _usdcOverride();
        if (usdcOverrideAddr == address(0)) {
            _checkTokenSymbol(chainId, "usdc", c.usdc, [string("USDC"), "", "", ""]);
        } else if (c.usdc == usdcOverrideAddr) {
            _pass("usdc", "address", string.concat(vm.toString(c.usdc), " (matches config.toml usdcOverride)"));
        } else {
            _fail(
                "usdc",
                "address",
                string.concat("expected config.toml usdcOverride ", vm.toString(usdcOverrideAddr))
            );
        }
