...
describe("Ethereum Spoke Pool", function () {
  ...
  it("Only owner can initialize a relayer refund", async function () {
    await expect(spokePool.connect(rando).relayRootBundle(rootBundleId, mockTreeRoot, mockTreeRoot)).to.be.reverted;
    await spokePool.connect(owner).relayRootBundle(rootBundleId, mockTreeRoot, mockTreeRoot);
    expect((await spokePool.rootBundles(rootBundleId)).slowRelayRoot).to.equal(mockTreeRoot);
    expect((await spokePool.rootBundles(rootBundleId)).relayerRefundRoot).to.equal(mockTreeRoot);
  });

  it("Only owner can delete a relayer refund", async function () {
    await spokePool.connect(owner).relayRootBundle(rootBundleId, mockTreeRoot, mockTreeRoot);
    await expect(spokePool.connect(rando).emergencyDeleteRootBundle(rootBundleId)).to.be.reverted;
    await expect(spokePool.connect(owner).emergencyDeleteRootBundle(rootBundleId)).to.not.be.reverted;
    expect((await spokePool.rootBundles(rootBundleId)).slowRelayRoot).to.equal(ethers.utils.hexZeroPad("0x0", 32));
    expect((await spokePool.rootBundles(rootBundleId)).relayerRefundRoot).to.equal(ethers.utils.hexZeroPad("0x0", 32));
  });
  ...
  it("Bridge tokens to hub pool correctly sends tokens to hub pool", async function () {
    ...
    await spokePool.connect(owner).relayRootBundle(rootBundleId, tree.getHexRoot(), mockTreeRoot);
    await expect(() =>
      spokePool.connect(relayer).executeRelayerRefundLeaf(rootBundleId, leaves[0], tree.getHexProof(leaves[0]))
