...
contract CounterfactualDepositTest is Test {
    ...
    function _depositOnlyTree(
        ...
        bytes32 depositLeaf = keccak256(
            bytes.concat(keccak256(abi.encode(block.chainid, address(cctpImpl), keccak256(depositParams))))
        );
        bytes32 dummyLeaf = keccak256(bytes.concat(keccak256(abi.encode(block.chainid, address(0), keccak256("")))));
    ...
    function _depositAndWithdrawTree(
        ...
        bytes32 withdrawLeaf = keccak256(
            bytes.concat(keccak256(abi.encode(block.chainid, address(withdrawImpl), keccak256(withdrawParams))))
        );
