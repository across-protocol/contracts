...
describe("Mode_Adapter", function () {
  ...
  it("Correctly calls Hyperlane XERC20 bridge", async function () {
    ...
    await adapterStore.connect(owner).setHypXERC20Router(modeChainId, ezETH.address, hypXERC20Router.address);
    adapterStore.hypXERC20Routers.whenCalledWith(modeChainId, ezETH.address).returns(hypXERC20Router.address);
