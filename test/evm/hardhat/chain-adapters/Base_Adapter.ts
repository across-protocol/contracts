...
describe("Base_Adapter", function () {
  ...
  it("Correctly calls Hyperlane XERC20 bridge", async function () {
    ...
    await adapterStore.connect(owner).setHypXERC20Router(baseChainId, ezETH.address, hypXERC20Router.address);
    adapterStore.hypXERC20Routers.whenCalledWith(baseChainId, ezETH.address).returns(hypXERC20Router.address);
