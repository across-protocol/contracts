...
describe("Unichain_Adapter", function () {
  ...
  beforeEach(async function () {
    ...
    await hubPool.setCrossChainContracts(unichainChainId, doctorWhoAdapter.address, mockSpoke.address);
    await hubPool.setPoolRebalanceRoute(unichainChainId, ezETH.address, l2EzETH);
  });
  ...
  it("Correctly calls Hyperlane XERC20 bridge", async function () {
    ...
    const { leaves, tree, tokensSendToL2 } = await constructSingleChainTree(ezETH.address, 1, unichainChainId);
