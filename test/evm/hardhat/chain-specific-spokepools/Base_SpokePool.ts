...
describe("Base Spoke Pool", function () {
  beforeEach(async function () {
    ...
    // Create the fake at the Base cross domain messenger and l2StandardBridge pre-deployment addresses.
    // Base uses the same addresses as Optimism for these contracts
    ...
    // Deploy Base_SpokePool
    baseSpokePool = await hre.upgrades.deployProxy(
    ...
    await seedContract(baseSpokePool, relayer, [dai, l2EzETH], weth, amountHeldByPool);
  ...
  it("Bridge tokens to hub pool correctly using the Hyperlane XERC20 messaging for ezETH token", async function () {
    ...
    const { leaves, tree } = await constructSingleRelayerRefundTree(
      l2EzETH.address,
      await baseSpokePool.callStatic.chainId(),
      ezETHSendAmount
    );

    // Set up admin permission to relay root bundle
    crossDomainMessenger.xDomainMessageSender.returns(owner.address);
    await baseSpokePool.connect(crossDomainMessenger.wallet).relayRootBundle(tree.getHexRoot(), mockTreeRoot);
    crossDomainMessenger.xDomainMessageSender.reset();
