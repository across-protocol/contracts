...
describe("Unichain Spoke Pool", function () {
  beforeEach(async function () {
    ...
    // Create the fake at the Unichain cross domain messenger and l2StandardBridge pre-deployment addresses.
    // Unichain uses the same addresses as Optimism for these contracts
    ...
    unichainSpokePool = await hre.upgrades.deployProxy(
      await getContractFactory("DoctorWho_SpokePool", owner),
    ...
    await seedContract(unichainSpokePool, relayer, [dai, l2EzETH], weth, amountHeldByPool);
  ...
  it("Bridge tokens to hub pool correctly using the Hyperlane XERC20 messaging for ezETH token", async function () {
    ...
    const { leaves, tree } = await constructSingleRelayerRefundTree(
      l2EzETH.address,
      await unichainSpokePool.callStatic.chainId(),
      ezETHSendAmount
    );

    // Set up admin permission to relay root bundle
    crossDomainMessenger.xDomainMessageSender.returns(owner.address);
    await unichainSpokePool.connect(crossDomainMessenger.wallet).relayRootBundle(tree.getHexRoot(), mockTreeRoot);
    crossDomainMessenger.xDomainMessageSender.reset();
