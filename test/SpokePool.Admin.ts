...
describe("SpokePool Admin Functions", async function () {
  ...
  it("Whitelist token path", async function() {
    ...
    expect(await spokePool.whitelistedDestinationRoutes(erc20.address, depositDestinationChainId)).to.equal(erc20.address)
  });
  it("Change deposit quote buffer", async function() {
    await expect(spokePool
      ...
      .setDepositQuoteTimeBuffer(
        60
      )).to.emit(spokePool, "SetDepositQuoteTimeBuffer").withArgs(
        60
      );
